package ohgiraffers.practice03.annotationconfig;

import ohgiraffers.common.BoardDTO;
import org.springframework.stereotype.Component;

import java.util.HashMap;
import java.util.Map;

@Component
public class BoardDAO {
    private final Map<Long,BoardDTO> boardMap = new HashMap<>();

    public BoardDAO(){
        boardMap.put(1L,new BoardDTO(1L,"스프링 수업 어때?","저는 새롭고 재밌네요","개발자 꿈나무"));
        boardMap.put(2L,new BoardDTO(2L,"스프링 수업 앞으로 어려워질까??","그래도 포기하지 않을거야!","미래의 멋진 개발자"));

    }

    public BoardDTO selectBoard(Long id){
        return boardMap.get(id);
    }

    public boolean insertBoard(BoardDTO newBoard){
        int before = boardMap.size();

        boardMap.put(newBoard.getId(),newBoard);

        int after = boardMap.size();

        return before<after;
    }
}
